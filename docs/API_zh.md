...
添加 TTF 字体。字体会自动子集化 — 输出 PDF 中仅嵌入文档实际使用的字形，有效控制文件大小。
...
设置当前字体。`style` 字符串接受 `""`、`"B"`、`"I"`、`"U"` 或组合如 `"BI"`。
...
- `x, y` — 矩形区域左上角坐标（文档单位）
- `w, h` — 矩形区域的宽度和高度（文档单位）
- `htmlStr` — 要渲染的 HTML 字符串
- `opt` — 渲染选项（参见 `HTMLBoxOption`）

**返回值：** 最后渲染内容之后的 Y 坐标，以及可能的错误。

**支持的标签：** `<b>`、`<strong>`、`<i>`、`<em>`、`<u>`、`<br>`、`<p>`、`<div>`、`<h1>`–`<h6>`、`<font>`、`<span>`、`<img>`、`<ul>`、`<ol>`、`<li>`、`<hr>`、`<center>`、`<a>`、`<blockquote>`、`<sub>`、`<sup>`

**支持的内联 CSS 属性**（通过 `style` 属性）：`color`、`font-size`、`font-family`、`font-weight`、`font-style`、`text-decoration`、`text-align`

**颜色格式：** `#RGB`、`#RRGGBB`、`rgb(r,g,b)`、CSS 命名颜色（black、red、blue 等）
